...
sub checkerror_laevis{
	...
	foreach my $fragmentid (sort {$a<=>$b} keys %{$transcript_hash{$QSEQID}}){
		...
		if ($firstfrag ne $comparefrag){
		    if ($firstfrag ne $comparefrag){
			    if ($firstfragname =~ "sca" && $comparefragnum <=18){ #case 1: scaffold >10 compare with chr/scaffold 1-10; no error but document it as containing scaffold; meanwhile, prevent it proceed with orientation check.  
			    ...
			    elsif(($firstfragnum <= 18 && $comparefragnum <=18)&&($firstfragnum != $comparefragnum)){#case 3: scaffold/chr1-10 compare with caffold/chr1-10 and it doesnt match; it is an error
...
if ($DoneLoading){
    foreach my $qseqid (sort keys %transcript_hash){ 
		if ((exists $tightseq_hash{$qseqid}) && ($tightseq_hash{$qseqid} eq "1")){ #if it is a transcript with tight, sort it in subroutine and error will be identified in subroutine too 
			sorttight_laevis($qseqid);
		}
		else{	
			checkerror_laevis($qseqid);		
		}
	}
}
...
print "Summary\n";
print "Total number of genes:", scalar keys %error_hash, " \n";
print "Number of error free assembled transcripts: $noerror (", $noerror/scalar keys %error_hash, ")\n";
print "There are $ChromosomeError genes that were assembled with different Chomosomes (", $$ChromosomeError/scalar keys %error_hash, ")\n";
print "There are $OrientationError genes that have orientation problems (", $OrientationError/scalar keys %error_hash, ") \n";
print "There are $OrderError genes that have order problem (", $OrderError/scalar keys %error_hash, ")\n";
