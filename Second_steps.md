...
#open the input file
my $infile = "C:/Users/Sarah/Desktop/Perl Practice/blastn100.txt"; 
...
while ( my $line = <INPUT>) {
	...
	if (exists $transcript_hash{$line[0]}){ #if the transcript with seqid already exist, assign it to a temperary hash for later comparison
		...
		$temptranscript_hash{$line[0]}{"fragment"}{"seqid"}[0]= $line[1]; 
		$temptranscript_hash{$line[0]}{"fragment"}{"qstart"}[0]= $line[6]; 
		$temptranscript_hash{$line[0]}{"fragment"}{"qend"}[0]= $line[7]; 
		$temptranscript_hash{$line[0]}{"fragment"}{"sstart"}[0] = $line[8];
		$temptranscript_hash{$line[0]}{"fragment"}{"send"}[0]= $line[9];
		$temptranscript_hash{$line[0]}{"fragment"}{"evalue"}[0]= $line[10];
		# print "the qend for the line stored in temp_hash is: ", $temptranscript_hash{$line[0]}{"fragment"}{"qend"}[0], "\n"; #check if temp-hash is functioning
		# print "the qstart and qend value for transcript hash and temp_hash respectively are: ",  
		# $transcript_hash{$line[0]}{"fragment",$numfragment}{"qstart"}[0],"\t", $transcript_hash{$line[0]}{"fragment",$numfragment}{"qend"}[0], "\t",
		# $temptranscript_hash{$line[0]}{"fragment"}{"qstart"}[0], "\t", $temptranscript_hash{$line[0]}{"fragment"}{"qend"}[0], "\n",; #check if the hashs are empty
		
		#store qend and qstart in variable for easier visual
		my $tempqstart = $temptranscript_hash{$line[0]}{"fragment"}{"qstart"}[0];
		my $tempqend = $temptranscript_hash{$line[0]}{"fragment"}{"qend"}[0];
		
		
		#foreach my $i(1..$numfragment){
		#store qend and qstart in variable for easier visual
		my $tqstart = $transcript_hash{$line[0]}{"fragment",$numfragment}{"qstart"}[0];
		my $tqend = $transcript_hash{$line[0]}{"fragment",$numfragment}{"qend"}[0];
		
		if (($tqstart <= $tempqstart)&& ($tqend >= $tempqend)){ #fragment comparison case1: repeated region; dont store it;
		    	%temptranscript_hash =(); #optional
		    	$tqstart = $tqend = $tempqstart =$tempqend = undef;
		    }
		  
		elsif ((($tqstart <= $tempqstart)&&($tqend <= $tempqend)) || #some overlap with the front
		     (($tqstart >= $tempqstart)&&($tqend >= $tempqend)) || #some overlap with the end
		    (($tqstart >= $tempqstart)&&($tqend <= $tempqend))){ # fragment comparison case2: some overlap region; compare e-value? sstart and send?;
		    $numfragment ++;
                    $transcript_hash{$line[0]}{"fragment",$numfragment}{"seqid"}[0]= $line[1]; 
                    $transcript_hash{$line[0]}{"fragment",$numfragment}{"qstart"}[0]= $line[6]; 
                    $transcript_hash{$line[0]}{"fragment",$numfragment}{"qend"}[0]= $line[7]; 
                    $transcript_hash{$line[0]}{"fragment",$numfragment}{"sstart"}[0] = $line[8];
                    $transcript_hash{$line[0]}{"fragment",$numfragment}{"send"}[0]= $line[9];
                    $transcript_hash{$line[0]}{"fragment",$numfragment}{"evalue"}[0]= $line[10];
                    if ($transcript_hash{$line[0]}{"fragment",$numfragment}{"sstart"}[0] < $transcript_hash{$line[0]}{"fragment",$numfragment}{"send"}[0]){
                        $transcript_hash{$line[0]}{"fragment",$numfragment}{"RorF"}[0] = 1;
                        }
                    else {
                          $transcript_hash{$line[0]}{"fragment",$numfragment}{"RorF"}[0] = -1;
                          }
		    ...
		    (($tqend <= $tempqstart)&&($tqend <= $tempqend))) { #no overlap at all; 
		        $numfragment = 1;
			$transcript_hash{$line[0]}{"fragment",$numfragment}{"seqid"}[0]= $line[1]; 
                        $transcript_hash{$line[0]}{"fragment",$numfragment}{"qstart"}[0]= $line[6]; 
                        $transcript_hash{$line[0]}{"fragment",$numfragment}{"qend"}[0]= $line[7]; 
	...
	print "the qend for the first fragment stored in the hash with qseqid ", $line[0], "is: ", $transcript_hash{$line[0]}{"fragment",$numfragment}{"qend"}[0], "\n"; #check if the hash if funtioning
	}
}	
close INPUT;

#output the content in hash
#print "\n"; print "\n"; print "\n"; print "\n";
#print Dumper(\%transcript_hash);

foreach my $qseqid (sort keys %transcript_hash){
    foreach my $fragmentid (sort keys %{$transcript_hash{$qseqid}}){
          print "$qseqid, $fragmentid, ";
        
        foreach my $dataname (sort keys %{$transcript_hash{$qseqid}{$fragmentid}}) {
          print $transcript_hash{$qseqid}{$fragmentid}{$dataname}[0], "\t";
          }
...
## improvement: 
#the fragment should compare to the all the fragments stored in the hash; 
#add a check for e-value, exclude fragments with e-value lower than ?
```
